...
function myFunction() {
   window.location.href="<?php echo $t ?>";
    ...
    $alertType ="alert-danger";
     ...
     if(mysqli_num_rows($result) == 0 || $transaction_form_level+1 == mysqli_num_rows($result)){
          echo " <script>
        window.location.href='transfer';
        </script>";
     }
    ...
    if (!empty($cleanData)) {
        ...
        move_uploaded_file($_FILES[$inputName]['tmp_name'], $targetFile);
         $sql = "UPDATE  `$table` SET value =  '$filename' , status ='submited' WHERE id = '$form_id'";
        ...
        if ($con->query($sql)) {
           ...
           $sql = "SELECT * from transaction_forms WHERE   user_id = '$user_id' && status = 'pending' LIMIT 0 , 1";
     $result = mysqli_query($con,$sql) or die("cant select members ".mysqli_error($con));
     $num_forms = mysqli_num_rows( $result );
     if($num_forms == 0){
        //compelted the procedure
        $sql = "UPDATE transactions SET status = 'processing' where user_id = '$user_id' && id = '$tid'";
